...
def perform_document_retrieval(
    ...
    vs = ss.get_vector_store(
        use_opensearch_vectorstore, embedding_model, knowledge_base
    )
...
def make_system_message(
    prompt: str, docs: List[Tuple[Document, float]], user_files: List[Tuple[str, str]],
) -> str:
...
def answer_query(
    ...
    try:
        ...
        chat_llm = ChatOpenAI(model_name=llm_model, temperature=0.0, streaming=True,)
